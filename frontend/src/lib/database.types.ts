    ...
    Tables: {
      accounts: {
        Row: {
          ...
          name: string; institution_name: string; account_identifier: string | null; notes: string | null; metadata: Json; tags: string[]; sort_order: number; deleted_at: string | null; created_at: string; updated_at: string
        }
        Insert: { id?: string; user_id: string; side: 'asset' | 'liability'; account_type: 'bank_account' | 'brokerage' | 'robo_advisor' | 'retirement_account' | 'digital_wallet' | 'crypto_wallet' | 'crypto_exchange' | 'rsu' | 'credit_card' | 'personal_loan' | 'other'; name: string; institution_name: string; account_identifier?: string | null; notes?: string | null; metadata?: Json; tags?: string[]; sort_order?: number; deleted_at?: string | null }

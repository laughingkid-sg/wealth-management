...
The active OCBC, Amex, and Citi prompt fragments share a source-only naming rule: `title` and optional `merchant_name` use only explicitly cited source text. OCBC NETS/NETS QR titles are `NETS · <merchant>`; outgoing PayNow titles are `PayNow to · <recipient>` and incoming titles are `PayNow from · <sender>`. When explicitly present in the same PayNow source, a masked mobile number, masked NRIC, or UEN is appended visibly to the title. Amex and Citi use the exact source merchant descriptor, while their card suffix remains `card_last_four` Account evidence. Payment references, approval codes, NETS IDs, and other provider transaction IDs are emitted in `references`; a masked mobile/NRIC or UEN is emitted in `account_evidence.additional_identifiers`. The latter is descriptive evidence only: reconciliation deliberately excludes it from Account matching. Updating this prompt configuration affects future parses and manual retries only; it never rewrites an existing canonical transaction title.
